...
export const MEETING_COLOR = '#e879f9'

/**
 * meetings[] → CalendarEvent[] (one per meeting, showing guest name + time)
 */
export function normalizeMeetingEvents(meetings) {
  return meetings
    ...
    .map(m => {
      ...
      const time  = start.toLocaleTimeString('ru-RU', { hour: '2-digit', minute: '2-digit', hour12: false })
      return {
        id:    `meeting-${m.id}`,
        date,
        type:  'meeting',
        title: `👤 ${m.guest_name} · ${time}`,
        color: MEETING_COLOR,
        refId: m.id,
        done:  false,
      }
